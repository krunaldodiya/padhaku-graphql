...
class CalculateQuizRanking implements ShouldQueue
{
    ...
    public function handle(QuizRepositoryInterface $quizRepo)
    {
        ...
        $quiz_started_participants = $quiz_data->participants()->where('quiz_status', 'finished')->count();
        ...
        $quiz_rankings = $quiz_participants->map(function ($quiz_participant, $index) use ($quiz_data) {
            ...
            return [
                ...
                'rank' => $rank,
                'prize_amount' => $contribution ? $contribution['price'] : 0,
                'created_at' => now(),
                'updated_at' => now(),
            ];
        });
        ...
        DB::table("quiz_participants")->where('quiz_status', '!=', 'finished')->update(['quiz_status' => 'missed']);
